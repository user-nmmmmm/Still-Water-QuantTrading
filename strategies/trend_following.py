...
class TrendUpStrategy(Strategy):
    ...
    def should_exit(self, symbol: str, i: int, df: pd.DataFrame, state: MarketState, portfolio: Portfolio) -> Optional[Dict[str, Any]]:
        ...
        current_trail = ctx.get('trailing_stop', -np.inf)
        # Assuming we only trail UP. If price drops, trail stays.
        # Logic: trail = max(previous_trail, close - 2*ATR)
        # But wait, if close drops, close-2*ATR drops. We want trail to only go up.
        # So: new_trail_candidate = close - 2*ATR. 
        # if new_trail_candidate > current_trail: current_trail = new_trail_candidate
        
        new_trail_candidate = close - 2 * atr
        if new_trail_candidate > current_trail:
        ...
        # Check against stop_loss (initial) and trailing_stop
        stop_loss = ctx.get('stop_loss', -np.inf)
        effective_stop = max(stop_loss, ctx['trailing_stop'])
...
class TrendDownStrategy(Strategy):
    ...
    def should_exit(self, symbol: str, i: int, df: pd.DataFrame, state: MarketState, portfolio: Portfolio) -> Optional[Dict[str, Any]]:
        ...
        # 3. Stop/Trail triggered
        stop_loss = ctx.get('stop_loss', np.inf)
        effective_stop = min(stop_loss, ctx['trailing_stop'])
