...
- **动态策略路由 (Dynamic Routing)**: 
  ...
  - **[New] Alpha 增强**: 在高波动 (Volatile) 状态下自动路由至 **Trend Breakout** 策略，捕捉爆发性行情。
- **机构级风控体系**:
...
针对 **Trend Breakout Alpha** 进行了 "System vs System+Alpha" 的严格对比验证（2025-2026 周期）：

| 指标 | 原系统 (Baseline) | 原系统 + Alpha | 提升幅度 |
| :--- | :--- | :--- | :--- |
| **年化收益 (CAGR)** | 2.21% | **9.69%** | **+338%** |
| **夏普比率 (Sharpe)** | 0.76 | **2.30** | **效率翻倍** |
| **卡尔玛 (Calmar)** | 1.14 | **2.54** | 风险收益比更佳 |
| **最大回撤 (MaxDD)** | -1.93% | -3.81% | 在可控范围内 |

> **结论**: Alpha 策略成功在 "强趋势启动/高波动" 阶段提供了显著的超额收益，验证了 P0-P5 研发流程的有效性。

---

## 🔬 研发流程 (P0-P5 Methodology)

本项目采用严格的 Alpha 研发至上线流水线，确保策略的鲁棒性：

1.  **P0 选题 (Direction)**: 明确 Alpha 类型（如 Trend Breakout）与非目标（不追求高胜率，只求特定 Regime 获利）。
2.  **P1 原型 (MVP)**: `research/` 下进行向量化裸回测，验证信号逻辑与存活能力 (Alive)。
3.  **P2 现实检验 (Reality Check)**: 引入真实成本 (Fee/Slip) 与极端行情压力测试 (2020-2024)。
4.  **P3 系统集成 (Integration)**: 封装为 `Strategy Plugin`，接入 `Router` 与 `RiskManager`。
5.  **P4 组合验证 (Portfolio)**: 对比 Baseline 与 Alpha 组合的绩效差异，确保 1+1 > 2。
6.  **P5 熔断机制 (Failure Mode)**: 定义自动失效判据（如 Rolling Sharpe < 0），实现优雅下线。
...
### 3. Alpha 策略：Trend Breakout (Donchian)
- **核心逻辑**: 捕捉价格突破 $N$ 日高点的爆发性动量。
- **入场**: $Price > Max(High, 20)$
- **出场**: $Price < Min(Low, 10)$
- **适用环境**: **VOLATILE** (ADX > 25) & **TREND_UP**
- **熔断机制**: 连续 5 笔亏损或滚动夏普小于 0 时自动停用。
...
│   └── trend_breakout.py   # [New] 趋势突破策略 (Alpha)
├── research/               # [Lab] Alpha 孵化实验室 (P1-P2)
│   ├── alpha_breakout.py   # 向量化原型验证
│   └── p2_reality_check.py # 成本压力测试
...
更多关于系统内部机制与部署操作的细节，请参考以下文档：

- **[回测假设与逻辑](docs/backtest_assumptions.md)**: 详细说明了订单执行模型、滑点/手续费计算及数据处理逻辑。
- **[部署与运维指南](docs/deployment.md)**: 包含服务器部署、API 配置、实盘监控及后台运行的最佳实践。
