...
本系统目前主要用于 **策略研究与逻辑验证**，与实盘交易存在以下关键差异：

1.  **Bar Close Bias (收盘价成交偏差)**:
    -   回测引擎假设信号产生于 Bar `i` 的收盘时刻，并能立即以该 Bar 的 `Close` 价格成交。
    -   **实盘差异**: 实盘中通常只能在下一根 Bar 的 `Open` 时刻成交，或承受滑点。这会导致回测收益略微高估。
2.  **滑点缺失 (No Slippage)**:
    -   当前回测引擎未设置滑点，仅计算了 0.1% 的手续费。在流动性差的市场或突破行情中，实际滑点可能显著影响收益。
3.  **状态无持久化 (No Persistence)**:
