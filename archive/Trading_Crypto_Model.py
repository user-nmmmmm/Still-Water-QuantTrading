...
def m3_initialize_bigquant_run(context):
    # 加密货币手续费设置
    # 假设每笔交易手续费为交易金额的 0.1% (千一)
    context.set_commission(PerOrder(buy_cost=0.001, sell_cost=0.001, min_cost=0))
...
m1 = M.input_features_dai.v6(
    sql="""-- 使用DAI SQL获取加密货币数据
-- 请确保 user_factor 表包含加密货币数据 (如 BTCUSDT, ETHUSDT 等)
SELECT
    position,
    date,
    instrument
FROM user_factor_crypto_data
""",
    m_cached=False,
    m_name="""m1"""
)
