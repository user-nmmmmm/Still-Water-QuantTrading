...
def in_trade_window_cn(dt_cn: datetime,
                       start_h=20, start_m=0,
                       end_h=8, end_m=0) -> bool:
    """
...
def session_id_cn(dt_cn: datetime,
                  start_h=20, start_m=0) -> str:
    """
...
def rebalance(context, data, ranker_prediction):
    ...
    if len(stock_now) > 0:
        ...
        held = ranker_prediction[ranker_prediction.instrument.apply(lambda x: x in stock_now)]
        # position 越大越差 -> 先卖 position 最大的
        held_sorted = held.sort_values("position", ascending=False)["instrument"].tolist()
...
def m3_handle_data_bigquant_run(context, data):
    ...
    in_window = in_trade_window_cn(
        dt_cn,
        context.window_start_h, context.window_start_m,
        context.window_end_h, context.window_end_m
    )
    ...
    if context.last_rebalance_dt_cn is not None:
        if (dt_cn - context.last_rebalance_dt_cn) < timedelta(minutes=context.rebalance_every_minutes):
            return
    ...
    if "dt" in context.my_data.columns:
        ...
        s_start = sid_date.replace(hour=context.window_start_h, minute=context.window_start_m, second=0, microsecond=0)
        s_end = (sid_date + timedelta(days=1)).replace(hour=context.window_end_h, minute=context.window_end_m, second=0, microsecond=0)

        df_sess = df[(df["_dt_cn"] >= s_start) & (df["_dt_cn"] < s_end) & (df["_dt_cn"] <= dt_cn)]
        if df_sess.empty:
            return

        latest_dt = df_sess["_dt_cn"].max()
        ranker_prediction = df_sess[df_sess["_dt_cn"] == latest_dt].sort_values("position")
    else:
        # 兼容：只有自然日 date 信号（会变成窗口内“同一天用同一套信号”）
        # 这里用 session_id（sid）对应的 date
        ranker_prediction = context.my_data[context.my_data["date"] == sid].sort_values("position")
